...
def run(enabledButterFlyFolderPath, disabledButterFlyFolderPath):
    ...
    plt.step(en_speed_x, speed, 'black', label="desired_speed")
    plt.plot(en_speed_x, en_speed_y, 'r', label='en_speed', linewidth=3)
    plt.plot(dis_speed_x, dis_speed_y, 'b', label='dis_speed', linewidth=3)
    plt.legend(loc='best')
    plt.xlabel('t')
    plt.ylim(-5, 1600)
    plt.xlim(0, 1500)
